...
class Word2Vec(BaseWordEmbeddingsModel):
    ...
    def __init__(self, sentences=None, size=100, alpha=0.025, window=5, min_count=5,
        ...
        self.vocabulary = Word2VecVocab(
            ...
            sorted_vocab=bool(sorted_vocab), null_word=null_word, power_alpha=power_alpha)
...
class Word2VecVocab(utils.SaveLoad):
    def __init__(self, max_vocab_size=None, min_count=5, sample=1e-3, sorted_vocab=True, null_word=0, power_alpha=0.75):
    ...
    def make_cum_table(self, wv, power=0.75, domain=2**31 - 1):
        ...
        """
        print('Alpha power is set to', self.power_alpha)
        vocab_size = len(wv.index2word)
        self.cum_table = zeros(vocab_size, dtype=uint32)
        # compute sum of all power (Z in paper)
        train_words_pow = 0.0
        for word_index in xrange(vocab_size):
            train_words_pow += wv.vocab[wv.index2word[word_index]].count**self.power_alpha
        cumulative = 0.0
        for word_index in xrange(vocab_size):
            cumulative += wv.vocab[wv.index2word[word_index]].count**self.power_alpha
